#This code explores quantum advantage in the CHSH game under noisy conditions, imposing differing kinds of qubit noise for comparison to Werner state noise. It outputs three illustrative plots. 
import numpy as np
...
#plot of heatmap w/imposed noise
...
def compute_advantage_and_eps(noise_model, shots_fid=10000, game_trials=15000):
    ...
    for _ in range(game_trials):
        ...
        q_reward += utility(a, b, x, y, beta1=beta1, beta2=beta2)
        c_reward += utility(cAlice(x), cBob(y), x, y, beta1=beta1, beta2=beta2)
...
# Sweeps for different kinds of noise
n_points = 30   
...
for lam in lambda_sweep:
    ...
    eps, adv = compute_advantage_and_eps(noise_model)
    eps_deph.append(eps)
    adv_deph.append(adv)
...
plt.plot(eps_paper, adv_paper, 'r--', linewidth=3, label='Paper: isotropic Werner')
...
plt.title(r'Quantum advantage vs. $\varepsilon$')
plt.yscale('log')
plt.ylim(1e-4, 0.12)
plt.xlim(0, 0.35)
