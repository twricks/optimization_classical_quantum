...
# ===================== NEW FIGURE: Quantum Advantage vs. Combined Infidelity ε =====================
# Paper's analytic isotropic curve (exact CHSH formula from the paper, Eq. 36)
def paper_advantage(eps):
    return ((1 - eps) * (1 / np.sqrt(2)) - 0.5) / 2

# Re-usable helper using ONLY your existing functions
def compute_advantage_and_eps(noise_model, iterations=3000, shots_fid=2000):
    sim = AerSimulator(noise_model=noise_model)
    
    eps_s = singlet_fidelity(sim, shots=shots_fid)          # your function
    eps_meas = 0.0
    ...
    c_reward = 0.0
    for _ in range(iterations):
        x, y = Referee(0.5)        # ← UNCORRELATED UNIFORM INPUTS: P(x,y)=1/4 exactly (what the paper uses)
        qc = QuantumCircuit(2, 2)
        qc.h(0)
        qc.cx(0, 1)
        qAlice(x, qc)
...
plt.plot(eps_paper, adv_paper, 'r--', linewidth=2.5, label='Paper: isotropic noise (analytic)')
plt.plot(eps_deph, adv_deph, 'o-', color='blue', linewidth=2, label='Simulation: pure dephasing (λ only)')
plt.plot(eps_amp, adv_amp, 's-', color='green', linewidth=2, label='Simulation: pure amplitude damping (γ only)')

plt.xlabel(r'Combined infidelity $\epsilon$')
plt.ylabel('Quantum advantage')
plt.title(r'Quantum advantage vs. $\epsilon$  (CHSH, $\beta_1=\beta_2=0$, uniform inputs $P(x,y)=1/4$)')
plt.grid(True, alpha=0.3)
plt.ylim(0, 0.11)
plt.xlim(0, 0.32)
plt.legend()
...
print("New Fig. 2(c) reproduction complete.")
print(f"Paper isotropic threshold (advantage = 0): ε ≈ {1-1/np.sqrt(2):.3f}")
